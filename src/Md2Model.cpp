...
void Md2Model::Normal(float *p1, float *p2, float *p3)
{
...
void Md2Model::load(char *filenme)
{
	FILE *fp;
	int length;
...
Md2Model::Md2Model(char *fName, char *textName)
{
	load(fName);
	textN = textName;
}

void Md2Model::Draw(int startFrame, int endFrame)
{
	if (!done)
	{
		tga texture(textN);
